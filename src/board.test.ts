...
function tmpFile(): string {
	return path.join(
		os.tmpdir(),
		`test-board-${Date.now()}-${Math.random().toString(36).slice(2)}.md`,
	);
}
...
describe("TaskBoard", () => {
	...
	describe("updateTask", () => {
		...
		it("updates the updatedAt timestamp", () => {
			...
			const _originalUpdated = task.updatedAt;
	...
	describe("listTasks", () => {
		...
		it("handles tag filter when some tasks have no tags", () => {
			...
			assert.ok(result.every((t) => t.tags?.includes("backend")));
		});
	});

	describe("getTasksByStatus", () => {
		it("groups tasks by their status", () => {
			const _t1 = board.addTask("Todo task");
