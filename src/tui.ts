...
export class TaskTUI {
	...
	private setupUI(): void {
		...
		const footer = blessed.box({
			...
			height: 3,
			content:
				"{center}{bold}↑/↓{/bold} Navigate | {bold}←/→{/bold} Switch Column | {bold}e{/bold} Edit | {bold}a{/bold} Add | {bold}d{/bold} Delete | {bold}s{/bold} Status | {bold}f{/bold} Filter | {bold}h{/bold} Help | {bold}q{/bold} Quit{/center}",
			tags: true,
			border: {
				type: "line",
			},
	...
	private setupKeyBindings(): void {
		...
		this.screen.key(["down", "j"], () => {
			const tasks = this.getFilteredTasksForColumn(
				COLUMNS[this.currentColumn].status,
			);
	...
	private render(): void {
		...
		header.setContent(
			`{center}{bold}{cyan-fg}${info.name}{/cyan-fg}{/bold}\n` +
	...
	private showAddTaskDialog(): void {
		...
		titlePrompt.input("Task Title (required):", "", (err, title) => {
			...
			descPrompt.input(
				...
				"",
				(_err, desc) => {
					taskData.description = desc?.trim() ? desc.trim() : undefined;
					...
					priorityPrompt.input(
						"Priority (low/medium/high/critical, default: medium):",
						"medium",
						(_err, priority) => {
							const validPriorities = ["low", "medium", "high", "critical"];
							taskData.priority = validPriorities.includes(priority?.trim())
								? priority.trim()
								: "medium";
	...
	private showEditTaskDialog(): void {
		...
		titlePrompt.input("Task Title:", task.title, (err, title) => {
			...
			descPrompt.input("Description:", task.description || "", (_err, desc) => {
				const newDesc = desc?.trim() ? desc.trim() : undefined;
				...
				priorityPrompt.input(
					...
					(_err, priority) => {
						const validPriorities = ["low", "medium", "high", "critical"];
						const newPriority =
							priority && validPriorities.includes(priority.trim())
								? priority.trim()
								: task.priority;
	...
	private showStatusMenu(): void {
		...
		const menu = blessed.list({
			...
			keys: true,
			vi: true,
			items: ["TODO", "IN PROGRESS", "DONE", "BLOCKED"],
		});
	...
	private showFilterDialog(): void {
		...
		blessed.text({
			...
			left: 2,
			content:
				"{bold}Filter tasks (title, description, assignee, tags):{/bold}",
			tags: true,
		});
