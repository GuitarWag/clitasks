...
const displayTask = (task: any, detailed: boolean = false): void => {
	...
	console.log(
		`  Status: ${formatStatus(task.status)} | Priority: ${formatPriority(task.priority)}`,
	);
	...
	if (task.tags && task.tags.length > 0) {
		console.log(
			`  Tags: ${task.tags.map((t: string) => chalk.magenta(`#${t}`)).join(" ")}`,
		);
	}
	...
	if (detailed) {
		console.log(
			`  ${chalk.dim(`Created: ${task.createdAt} | Updated: ${task.updatedAt}`)}`,
		);
	}
};
...
program
	...
	.action((options) => {
		...
		for (const task of tasks) {
			displayTask(task, options.detailed);
		}
	});
			...
			if (tasks.length === 0) {
				...
				tasks.forEach((task) => {
					...
					const assignee = task.assignee
						? chalk.green(`@${task.assignee}`)
						: "";
					const tags = task.tags
						? task.tags.map((t) => chalk.magenta(`#${t}`)).join(" ")
						: "";
					console.log(
						`  ${chalk.cyan(task.id)} ${task.title} ${priority} ${assignee} ${tags}`,
					);
					if (task.description) {
						console.log(chalk.dim(`    ${task.description}`));
					}
...
program
	...
	.action(() => {
		...
		console.log(
			`  IN PROGRESS: ${chalk.blue(tasksByStatus["in-progress"].length)}`,
		);
		...
		if (assignees.size > 0) {
			...
			Array.from(assignees.entries())
				...
				.forEach(([assignee, count]) => {
					console.log(
						`  ${chalk.green(assignee)}: ${count} task${count > 1 ? "s" : ""}`,
					);
				});
			console.log("");
		}
		...
		const completionRate =
			info.tasks.length > 0
				...
				: 0;
		console.log(
			chalk.bold(`Completion Rate: ${chalk.cyan(`${completionRate}%`)}\n`),
		);
	});
	...
	.action((options) => {
		...
		switch (options.format) {
			...
			default:
				console.log(
					chalk.red(
						`✗ Invalid format: ${options.format}. Use json, csv, or summary.`,
					),
				);
				return;
		}
...
program
	...
	.action((options) => {
		...
		if (!skillSourcePath) {
			console.log(
				chalk.red(
					"✗ SKILL.md not found. Make sure tasks-cli is properly installed.",
				),
			);
			process.exit(1);
		}
		...
		try {
			...
			if (options.global) {
				console.log(
					chalk.green(`✓ SKILL.md installed to global Claude skills:`),
				);
				...
				console.log(
					chalk.green(`✓ SKILL.md installed to local Claude skills:`),
				);
				console.log(chalk.cyan(`  ${targetPath}`));
				console.log(
...
program
	...
	.action((options) => {
		...
		try {
			...
			if (options.global) {
				console.log(
					chalk.green(`✓ SKILL.md installed to global Codex skills:`),
				);
				console.log(chalk.cyan(`  ${targetPath}`));
